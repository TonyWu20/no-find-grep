...
A [Claude Code](https://claude.ai/code) plugin that blocks bare `find`/`grep` in Bash calls and detects common `rg` flag misuse.

> Shell file-read and file-edit enforcement has moved to the sibling plugin:
> **[no-shell-file-ops](../no-shell-file-ops)** — blocks `sed`, `cat`, `xxd`, `od`, `sd`, `python -c` file operations and provides Edit tool stale-string hints.
...
> Always use `fd` and `rg` over `find` and `grep`.
